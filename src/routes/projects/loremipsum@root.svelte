<script lang="ts">
	...
	onMount(() => {
		const form = document.querySelector<HTMLFormElement>('.lorem-form') as HTMLFormElement;
		const amount = document.getElementById('amount') as HTMLInputElement;
		const result = document.querySelector('.lorem-text') as HTMLElement;
		...
		form.addEventListener('submit', (e) => {
			...
			if (isNaN(value) || value < 0 || value > 9) {
				result.innerHTML = `<p stlye="margin-bottom: 2rem"> ${text[random]}</p>`;
			} else {
				let tempText = text.slice(0, value);
				let formatedText = tempText
					.map((item) => {
						return `<p stlye="margin-bottom: 2rem"> ${item}</p>`;
					})
					.join('');
